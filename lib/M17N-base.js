	...
	for(var pi in pList)
	{
		...
		switch(matchMode)
		{
			...
			case 1:console.log("one and only one match possible");				
				...
				this.mim.tempBuffer=this.mim.preCommitBuffer;
				done=true;
				break;
			case 2:console.log("match possible but more matches possible");
				...
				this.mim.pushState()
				this.doActions(p,chr,match);
				var temp=this.mim.parsedBuffer+this.mim.preCommitBuffer;
				this.mim.popState()
				this.mim.tempBuffer=temp.slice(this.mim.parsedBuffer.length)
				break;
...
MIM.prototype.backSpace=function()
{
	console.log(this.parsedBuffer,this.preCommitBuffer)
	if(this.preCommitBuffer.length>0 || this.tempBuffer.length>0)
	{
		this.preCommitBuffer="";
		this.tempBuffer="";
		this.shiftState(this.initState);
		this.releaseHandler();
	}
	else
	{
		this.shiftState(this.initState);
		this.parsedBuffer=this.parsedBuffer.slice(0,this.parsedBuffer.length-1);
	}
	console.log(this.parsedBuffer,this.preCommitBuffer)
}
...
MIM.prototype.pushState=function()
{
	console.log("pushing state")
	this.mimStates.push({"handler":this.currentHandler,"preCommitBuffer":this.preCommitBuffer,"parsedBuffer":this.parsedBuffer})	
}
MIM.prototype.popState=function()
{
	console.log("popping state")
	var st=this.mimStates.pop()
	this.currentHandler=st.handler;
	this.preCommitBuffer=st.preCommitBuffer;
	this.parsedBuffer=st.parsedBuffer;
}
